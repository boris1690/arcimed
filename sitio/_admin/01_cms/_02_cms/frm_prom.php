	...
	<!-- 
		...
		function funValidaDatos(dml,parECodiProm)
		{
			...
			if (parECodiProm == "")
			{
				...
				// foto 1
				if (dml.elements['txtFotoNorm'].value.length==0)
				{
					alert ('No ha ingresado la FOTO normal.');
					dml.elements['txtFotoNorm'].focus();
					return false;
				}
				else
				{
					varValoCamp = dml.elements['txtFotoNorm'].value;
					varExteArch = varValoCamp.substring(varValoCamp.length-3,varValoCamp.length).toUpperCase();
					if ((varExteArch == "ASP")||(varExteArch == "PHP")||(varExteArch == "JSP")||(varExteArch == "EXE")||(varExteArch == "BAT")||(varExteArch == "COM"))
					{
						alert ('El TIPO DE ARCHIVO SELECCIONADO no está permitido.');
			...
			// alt foto normal
			if (dml.elements['txtAltfNorm'].value.length==0)
			{
				alert ('No ha ingresado el ALT de la foto normal');
				dml.elements['txtAltfNorm'].focus();
				return false;
			}
			// alt foto grande
...
//			}
		...
		{
			// Recargo la pagina
			document.location = "./frm_prom.php";
			return false;
		}
...
<form action="./exe_prom.php" target="ejec" method="post" enctype="multipart/form-data">
...
<? 
	...
	{
		// Datos del Registro
		$recordset = funEjecutaQuerySelect ("select * from ".$aplPrefClie."_dat_promocion where COD_PROMOCION=$varCodiProm");
		$nr = funDevuelveArregloRecordset ($recordset,$results);
		if ($nr>0)
		{
			$varNombProm = trim($results["NOM_PROMOCION"][0]);
			$varFotoPequ = trim($results["FOT_PEQUPROMOCION"][0]);
			$varFotoNorm = trim($results["FOT_NORMPROMOCION"][0]);
			$varFotoGran = trim($results["FOT_GRANPROMOCION"][0]);
			$varTextProm = trim($results["TEX_PROMOCION"][0]);
			$varEstaProm = trim($results["EST_PROMOCION"][0]);
			$varAltfPequ = trim($results["ALT_PEQUPROMOCION"][0]);
			$varAltfNorm = trim($results["ALT_NORMPROMOCION"][0]);
			$varAltfGran = trim($results["ALT_GRANPROMOCION"][0]);
			$varLinkProm = trim($results["LIN_PROMOCION"][0]);
		}
...
<tr>
	<td align="center">
		...
		<tr>
			<td class="label" align="right">Foto Normal : &nbsp;</td>
			<td class="label"><input type="file" name="txtFotoNorm" value="" maxlength="60" style="width:389px;" class="textbox">&nbsp(<?=$varMaxiFileSize?> MB)</td>
		</tr>
		<tr>
		...
		<tr>
			<td class="label" align="right">Alt Foto Normal : &nbsp;</td>
			<td><input type="text" name="txtAltfNorm" value="<?= $varAltfNorm ?>" maxlength="60" style="width:389px;" class="textbox"></td>
		</tr>
		<tr>
		...
		<tr>
			<td class="label" align="right">Link : &nbsp;</td>
			<td><input type="text" name="txtLinkProm" value="<?= $varLinkProm ?>" maxlength="250" style="width:389px;" class="textbox"></td>
		</tr>
		<tr>
...
<tr>
	...
	<? } else { ?>
		...
		<input type=submit name=cmdComando value="Eliminar" style="width:90px;" onClick="return confirm('Esta seguro que desea eliminar la Promocion?');">
		<input type=submit name=cmdComando value="Limpiar" style="width:90px;" onClick="return funLimpiaDatos();">
	<? } ?>
	</td>
</tr>
