...
<? 
	...
	if ($varComando == "Ingresar")
	{
		...
		// ingreso el registro
		$recordset = funEjecutaQueryEdit ("insert into " . $aplPrefClie . "_dat_categoria values ($varCodiCate,'$varNombCate','$varEstaCate')");
	...
	if ($varComando == "Actualizar")
	{
		...
		$recordset = funEjecutaQueryEdit ("update " . $aplPrefClie . "_dat_categoria set NOM_CATEGORIA='$varNombCate', EST_CATEGORIA='$varEstaCate' where COD_CATEGORIA=$varCodiCate");
	...
	$recordset = funEjecutaQuerySelect ("select * from " . $aplPrefClie . "_dat_categoria order by NOM_CATEGORIA");
	$nr = funDevuelveArregloRecordset ($recordset,$results);
	if ($nr>0) 
	{
		// crea la tabla
		...
		for ($varI=0;$varI<$nr;$varI++)
		{
			...
			// despliego los datos
			print "<tr>";
			print "	<td width=\"97%\" class=\"texttabl\"><a href=\"./frm_cate.php?txtCodiCate=$varCodiCate\" target=\"form\" class=\"linktabl\">$varNombCate</a>&nbsp;<font style=\"color:#666666\"><i>$varEstaCateNomb</i></font></td>";
