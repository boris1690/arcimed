...
<? 
	...
	if ($varComando == "Ingresar")
	{
		...
		if ((($varSizePequ <= $aplMaxiFileSize)&&($varSizePequ > 0)) && (($varSizeGran <= $aplMaxiFileSize)&&($varSizeGran > 0)) && (($varSizeNorm <= $aplMaxiFileSize)&&($varSizeNorm > 0)))
		{
			// obtengo el codigo ingresado
			$recordset = funEjecutaQuerySelect ("select max(COD_PROMOCION) as COD_PROMOCION from ".$aplPrefClie."_dat_promocion");
			$nr = funDevuelveArregloRecordset ($recordset,$results);
			if ($nr>0) $varCodiProm = $results["COD_PROMOCION"][0] + 1; else $varCodiProm = 1;
			funLiberaRecordset ($recordset);

			// ingreso el registro
			$recordset = funEjecutaQueryEdit ("insert into " . $aplPrefClie . "_dat_promocion values ($varCodiProm,'$varNombProm','$varPathPequ','$varPathGran','$varPathNorm','$varAltfPequ','$varAltfGran','$varAltfNorm','$varLinkProm','$varTextProm','$varEstaProm')");
			...
			funEjecutaQueryEdit ("insert into " . $aplPrefClie . "_ref_consulta values ('$varUserUsua',$varCodiCons,'$varFechCons','Ingreso Promocion','$varNombProm')");
	...
	if ($varComando == "Actualizar")
	{
		...
		//{
			...
			$recordset = funEjecutaQueryEdit ("update " . $aplPrefClie . "_dat_promocion set NOM_PROMOCION='$varNombProm', ALT_PEQUPROMOCION='$varAltfPequ', ALT_GRANPROMOCION='$varAltfGran', ALT_NORMPROMOCION='$varAltfNorm', TEX_PROMOCION='$varTextProm', EST_PROMOCION='$varEstaProm', LIN_PROMOCION='$varLinkProm' where COD_PROMOCION=$varCodiProm");


			if ($varPathPequ!="")
			{
				$recordset = funEjecutaQueryEdit ("update " . $aplPrefClie . "_dat_promocion set FOT_PEQUPROMOCION='$varPathPequ' where COD_PROMOCION=$varCodiProm");
				// cargo el archivo
				if (is_uploaded_file($varPequTemp)) copy($varPequTemp,$aplUploadPathArch.$varPathPequ);
			}
			...
			if ($varPathNorm!="")
			{
				$recordset = funEjecutaQueryEdit ("update " . $aplPrefClie . "_dat_promocion set FOT_NORMPROMOCION='$varPathNorm' where COD_PROMOCION=$varCodiProm");
				// cargo el archivo
				if (is_uploaded_file($varNormTemp)) copy($varNormTemp,$aplUploadPathArch.$varPathNorm);
			}
			...
			if ($varPathGran!="")
			{
				$recordset = funEjecutaQueryEdit ("update " . $aplPrefClie . "_dat_promocion set FOT_GRANPROMOCION='$varPathGran' where COD_PROMOCION=$varCodiProm");
				// cargo el archivo
				if (is_uploaded_file($varGranTemp)) copy($varGranTemp,$aplUploadPathArch.$varPathGran);
			}
			...
			$varFechCons = date("Y-m-d H:i:s");
			$varUserUsua = $_SESSION['User_User'];
			funEjecutaQueryEdit ("insert into " . $aplPrefClie . "_ref_consulta values ('$varUserUsua',$varCodiCons,'$varFechCons','Actualizacion Promocion','$varNombProm')");
	...
	if ($varComando == "Eliminar")
	{
		...
		else
		{
			...
			$recordset = funEjecutaQuerySelect("select NOM_PROMOCION from " . $aplPrefClie . "_dat_promocion where COD_PROMOCION=$varCodiProm");
			$nr = funDevuelveArregloRecordset ($recordset,$results);
			if ($nr>0) $varNombProm = $results["NOM_PROMOCION"][0] ; else $varNombProm = "";
			funLiberaRecordset ($recordset);
			
			// elimina el registro
			$recordset = funEjecutaQueryEdit ("delete from " . $aplPrefClie . "_dat_promocion where COD_PROMOCION=$varCodiProm");
			...
			$varFechCons = date("Y-m-d H:i:s");
			$varUserUsua = $_SESSION['User_User'];
			funEjecutaQueryEdit ("insert into " . $aplPrefClie . "_ref_consulta values ('$varUserUsua',$varCodiCons,'$varFechCons','Eliminacion Promocion','$varNombProm')");
	...
	if ($nr>0) 
	{
		...
		for ($varI=0;$varI<$nr;$varI++)
		{
			...
			// despliego los datos
			print "<tr>";
			print "	<td width=\"97%\" class=\"texttabl\"><a href=\"./frm_prom.php?txtCodiProm=$varCodiProm\" target=\"form\" class=\"linktabl\">$varNombProm</a>&nbsp;<font style=\"color:#666666\"><i>$varEstaPromNomb</i></font></td>";
			print "	<td width=\"3%\" class=\"texttabl\" align=\"center\"><a href=\"./exe_prom.php?txtCodiProm=$varCodiProm&cmdComando=Eliminar\" onClick=\"return confirm('Esta seguro que desea eliminar el Registro?');\" class=\"linktabl\"><img src=\"./../images/btnx.gif\" width=\"12\" height=\"12\" border=0></a></td>";
			print "</tr>";
		}
