...
<? 	
	...
	$recordset = funEjecutaQuerySelect ("select A.*, B.NOM_CATEGORIA,C.NOM_CIUDAD from " . $aplPrefClie . "_dat_subcategoria A JOIN " . $aplPrefClie . "_dat_categoria B ON A.COD_CATEGORIA=B.COD_CATEGORIA JOIN " . $aplPrefClie . "_ref_ciudad C ON B.COD_CIUDAD = C.COD_CIUDAD order by C.COD_CIUDAD,B.NOM_CATEGORIA, A.NOM_SUBCATEGORIA");
	$nr = funDevuelveArregloRecordset ($recordset,$results);
	if ($nr>0) 
	{
		// crea la tabla
		...
		for ($varI=0;$varI<$nr;$varI++)
		{
			...
			$varCiudCate = trim($results["NOM_CIUDAD"][$varI]);
			...
			// despliego ciudad
			if ($varNombCiudAux != $varCiudCate)
			{
				print "<tr>";
				print "	<td colspan=\"2\" class=\"texttabl\" bgcolor=\"$aplTigraBackColorOver\"><font style=\"color:#ca6500\">" . $varCiudCate . "</font></td>";
				print "</tr>";
				$varNombCiudAux = $varCiudCate;
			}
			// despliego titulo de seccion
			if ($varNombCateAux != $varNombCate)
			{
				print "<tr>";
				print "	<td colspan=\"2\" class=\"texttabl\" style=\"color:#004080;padding-left:15px;\" bgcolor=\"#e4e3cf\">" . $varNombCate . "</td>";
				print "</tr>";
			...
			// despliego los datos
			print "<tr>";
			print "	<td width=\"97%\" class=\"texttabl\" style=\"padding-left:25px;\"><a href=\"./frm_subc.php?txtCodiSubc=$varCodiSubc\" target=\"form\" class=\"linktabl\">$varNombSubc</a>&nbsp;<font style=\"color:#666666\"><i>$varEstaSubcNomb</i></font></td>";
