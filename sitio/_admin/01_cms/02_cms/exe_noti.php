...
<? 
	...
	if ($varComando == "Ingresar")
	{
		...
		if ((($varSizePequ <= $aplMaxiFileSize)&&($varSizePequ > 0)) && (($varSizeGran <= $aplMaxiFileSize)&&($varSizeGran > 0)))
		{
			...
			// ingreso el registro
			$recordset = funEjecutaQueryEdit ("insert into " . $aplPrefClie . "_dat_noticia values ($varCodiNoti,'$varTituNoti','$varExtrNoti','$varTextNoti','$varPathPequ','$varPathGran','$varAltfPequ','$varAltfGran','$varEstaNoti',now())");
