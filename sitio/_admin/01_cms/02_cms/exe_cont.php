...
<? 
	...
	{
		$varCodiInfo = trim($_POST["txtCodiInfo"]);
		$varTituInfo = convertir_especiales_html(trim($_POST["txtTituInfo"]));
		$varFrasInfo = convertir_especiales_html(trim($_POST["txtFrasInfo"]));
		$varTextInfo = convertir_especiales_html(trim($_POST["txtTextInfo"]));
		$varAltfInfo = convertir_especiales_html(trim($_POST["txtAltfInfo"]));
		$varVideInfo = trim($_POST["txtVideInfo"]);

		$varFotoTemp = $_FILES['txtFotoInfo']['tmp_name'];
		$varPathFoto = $_FILES['txtFotoInfo']['name'];
		$varTipoFoto = $_FILES['txtFotoInfo']['type'];
		$varSizeFoto = $_FILES['txtFotoInfo']['size'];
		$varExteFoto = substr($varFotoTemp,-3);
	}
	...
	if ($varComando == "Actualizar")
	{
		...
		$recordset = funEjecutaQueryEdit ("update " . $aplPrefClie . "_dat_infoseccion set NOM_SECCION='$varTituInfo', VID_SECCION='$varVideInfo', FRA_SECCION='$varFrasInfo', TEX_SECCION='$varTextInfo', ALT_SECCION='$varAltfInfo' where COD_SECCION=$varCodiInfo");

		// cargo el archivo
		if ($varPathFoto != "")	
		{
			$recordset = funEjecutaQueryEdit ("update " . $aplPrefClie . "_dat_infoseccion set FOT_SECCION='$varPathFoto' where COD_SECCION=$varCodiInfo");
			if (is_uploaded_file($varFotoTemp)) copy($varFotoTemp,$aplUploadPathArch.$varPathFoto);
		}
		...
		if ($nr>0) $varNombSecc = $results["NOM_SECCION"][0]; else $varNombSecc = "";
		...
		funEjecutaQueryEdit ("insert into ".$aplPrefClie."_ref_consulta values ('$varUserUsua',$varCodiCons,'$varFechCons','Actualizacion Seccion','$varNombSecc')");

		// mensaje de exito
		funLevantaAlert ('El contenido fue ACTUALIZADO exitosamente');
	}
		...
		for ($varI=0;$varI<$nr;$varI++)
		{
			...
			if ($varNiveInfo == "1")
			{
				...
				if (($varTipoInfo == "L")||($varTipoInfo == "H"))
					print "	<td width=\"100%\" style=\"color:#000099\" class=\"linktabl\"><a href=\"./frm_cont.php?txtCodiInfo=$varCodiInfo\" target=\"form\" style=\"color:#000099\" class=\"linktabl\">$varNombInfo</a> $varIconEsta</td>";
				else
			...
			for ($varJ=0;$varJ<$ns;$varJ++)
			{
				...
				elseif ($varNiveSubs == "2")
				{
					print "<tr>";
					if (($varTipoSubs == "L")||($varTipoSubs == "H"))
						print "	<td width=\"100%\" style=\"color:#000000;padding-left:20px;\" class=\"texttabl\"><a href=\"./frm_cont.php?txtCodiInfo=$varCodiSubs\" target=\"form\" style=\"color:#000000\" class=\"linktabl\">$varNombSubs</a> $varIconEsta</td>";
					else
