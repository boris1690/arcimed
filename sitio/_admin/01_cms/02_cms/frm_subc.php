	...
	<td align="center">
		...
		<tr>
			...
			<td>
				...
				<?
					$recordset = funEjecutaQuerySelect ("SELECT * from ".$aplPrefClie."_dat_categoria A JOIN ".$aplPrefClie."_ref_ciudad B ON A.COD_CIUDAD=B.COD_CIUDAD where A.EST_CATEGORIA='A' order by B.NOM_CIUDAD");
					$nr = funDevuelveArregloRecordset ($recordset,$results);
					for ($varI=0;$varI<$nr;$varI++)
					{
						if ($varCodiCate == $results["COD_CATEGORIA"][$varI])
						{
							print "		<option value=\"" . $results["COD_CATEGORIA"][$varI] . "\" selected>" . trim($results["NOM_CATEGORIA"][$varI]) . " (".$results["NOM_CIUDAD"][$varI].")";
						}
						else
						{
							print "		<option value=\"" . $results["COD_CATEGORIA"][$varI] . "\">" . trim($results["NOM_CATEGORIA"][$varI]). " (".$results["NOM_CIUDAD"][$varI].")";
						}
					}
					funLiberaRecordset ($recordset);
				?>				
